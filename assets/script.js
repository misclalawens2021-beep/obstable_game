...
let currentLevel = 1;
let lives = 10;        // Changé de 3 à 10
let maxLives = 10;     // Changé de 3 à 10
...
function makeLevel(n) {
  // Génération procédurale : difficulté qui augmente avec n
  const p = {
    count: 12 + n * 2,
    baseW: 150 - n * 6,
    up: 55 + n * 3,
  ...
  // Pièges (pointes) : à partir du niveau 2
  const traps = [];
  if (n >= 2) {
  ...
  // Ennemis qui patrouillent : dès le niveau 1
  const enemies = [];
...
function loadLevel(n) {
  ...
  // === RÉINITIALISATION DES VIES À CHAQUE NOUVEAU NIVEAU ===
  lives = maxLives;
  updateLives();
  
  // Réinitialiser le checkpoint au début du niveau
  checkpoint = null;
...
function updateLives() {
  // Affiche les coeurs pleins et vides
  let hearts = '';
...
function looseLife() {
  ...
  if (lives <= 0) {
    ...
    // Si un checkpoint existe, on y retourne, sinon on respawn au début
    if (checkpoint && checkpoint.level === currentLevel) {
      respawnAtCheckpoint();
    } else {
      respawn();
  ...
  for (const p of level.platforms) {
    if (rectsCollide(player, p)) {
      if (player.vy > 0 && player.y + player.h - p.y < 15) {
        ...
        // === DÉFINIR UN CHECKPOINT QUAND LE JOUEUR TOUCHE UNE PLATEFORME ===
        // On ne définit pas de checkpoint sur la première plateforme de spawn
        if (p.x > 100) {
          setCheckpoint();
        }
...
function draw() {
  ...
  // === AFFICHAGE DU CHECKPOINT (petit drapeau) ===
  if (checkpoint && checkpoint.level === currentLevel) {
